...
class BoundedSendOperatorTests(unittest.TestCase):
    runner_module = "run_bounded_send_ab"
    first_variant = "baseline"
    modes = ("success", "runner-failure", "SIGINT", "SIGTERM", "restore-first-fails")
    def test_exit_signals_and_independent_restoration(self):
        ...
        for mode in self.modes:
         with tempfile.TemporaryDirectory() as d:
          ...
          proc=subprocess.Popen([sys.executable,str(base/'run_capacity_operator.py'),'--output-directory',str(root/'operator'),'--',sys.executable,str(driver)],env=env,stdout=subprocess.DEVNULL,stderr=subprocess.PIPE,text=True)
          ids={}
          try:
           if mode.startswith('SIG'):
