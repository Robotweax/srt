...
TEST(compat_runtime_reports_the_next_paced_send_deadline)
{
    ...
    REQUIRE(!first.paced_poll_deadline.has_value());
...
struct DatagramChannelTestAccess {
    using Time = PacedPollContinuation::Time;
    static RuntimePollResult poll(DatagramChannel& channel, Time now)
    {
        return channel.run_once(&now);
    }
    static void arm(DatagramChannel& channel, Time now, Time deadline)
    {
        channel.paced_poll_continuation_.arm(deadline, now,
            channel.paced_poll_epoch_.load(std::memory_order_acquire));
    }
    static bool take(DatagramChannel& channel, Time now)
    {
        return channel.paced_poll_continuation_.take(
            now, channel.paced_poll_epoch_.load(std::memory_order_acquire));
    }
};
} // namespace robotweax::srt::compat

TEST(compat_paced_continuation_is_one_shot_and_never_skips_a_due_deadline)
{
    using namespace std::chrono_literals;
    using Hint = PacedPollContinuation;
    const Hint::Time start {1s};
    Hint hint;
    hint.arm(start + 4us, start, 7U);
    REQUIRE(hint.take(start + 3us, 7U));
    REQUIRE(!hint.take(start + 3us, 7U));
    hint.arm(start + 4us, start, 7U);
    REQUIRE(!hint.take(start + 4us, 7U));
    hint.arm(start + 1ms, start, 7U);
    REQUIRE(!hint.take(start + 5us, 7U));
    hint.arm(start + 1ms, start, 7U);
    REQUIRE(!hint.take(start + 1s, 7U));
    hint.arm(start, start, 7U);
    REQUIRE(!hint.pending());
}

TEST(compat_paced_continuation_rejects_changed_work_and_clock_rollback)
{
    using namespace std::chrono_literals;
    using Hint = PacedPollContinuation;
    const Hint::Time start {1s};
    Hint hint;
    hint.arm(start + 4us, start, 7U);
    REQUIRE(!hint.take(start + 1us, 8U));
    REQUIRE(!hint.take(start + 1us, 7U));
    hint.arm(start + 4us, start, 7U);
    REQUIRE(!hint.take(start - 1us, 7U));
    hint.arm(Hint::Time::max(), Hint::Time::max() - 1us, 7U);
    REQUIRE(!hint.pending());
    hint.arm(Hint::Time::min(), start, 7U);
    REQUIRE(!hint.pending());
}

TEST(compat_paced_continuation_services_inbound_shutdown_on_next_full_poll)
{
    using namespace std::chrono_literals;
    const PacedPollContinuation::Time start {1s};
    auto channel = std::make_shared<DatagramChannel>();
    UdpSocket peer;
    const Ipv4Endpoint loopback {.address = {127, 0, 0, 1}};
    REQUIRE_EQ(channel->socket.bind(loopback), Error::none);
    REQUIRE_EQ(peer.bind(loopback), Error::none);
    std::uint64_t now = 1'000;
    auto runtime =
        std::make_shared<ConnectionRuntime>(ConnectionRuntime::Configuration {
            .channel = channel,
            .peer = peer.local_endpoint().endpoint,
            .peer_socket_id = 99U,
            .initial_sequence = SequenceNumber {100},
            .origin = ConnectionRuntime::Clock::now(),
            .now_function = injected_now,
    ...
    REQUIRE(channel->register_connection(42U, runtime));
    std::array<std::byte, 64> bytes {};
    const std::array<std::byte, 4> zero_word {};
    const auto encoded = encode_packet(
        MutablePacketView {
            .kind = PacketKind::control,
            .control = {.type = ControlType::shutdown,
                .destination_socket_id = 42U},
            .payload = zero_word,
        },
        bytes);
    REQUIRE(encoded);
    REQUIRE(peer.send_to(std::span {bytes}.first(encoded.bytes_written),
        channel->socket.local_endpoint().endpoint));
    REQUIRE(channel->socket.wait_readable(500).ready);
    DatagramChannelTestAccess::arm(*channel, start, start + 4us);
    REQUIRE(
        DatagramChannelTestAccess::poll(*channel, start + 1us).immediate_work);
    REQUIRE(!runtime->peer_closed());
    // Same timestamp: the consumed hint cannot renew or defer control twice.
    (void)DatagramChannelTestAccess::poll(*channel, start + 1us);
    REQUIRE(runtime->peer_closed());
}

TEST(compat_paced_continuation_new_send_work_forces_full_poll)
{
    using namespace std::chrono_literals;
    const PacedPollContinuation::Time start {1s};
    auto channel = std::make_shared<DatagramChannel>();
    REQUIRE_EQ(channel->socket.bind(Ipv4Endpoint {.address = {127, 0, 0, 1}}),
        Error::none);
    CapturedDatagrams output;
    channel->set_send_hook_for_testing(capture_datagram, &output);
    std::uint64_t now = 1'000;
    auto runtime =
        std::make_shared<ConnectionRuntime>(ConnectionRuntime::Configuration {
            .channel = channel,
            .peer = {.address = {127, 0, 0, 1}, .port = 99},
            .peer_socket_id = 99U,
            .initial_sequence = SequenceNumber {100},
            .origin = ConnectionRuntime::Clock::now(),
            .now_function = injected_now,
    ...
    REQUIRE(channel->register_connection(42U, runtime));
    DatagramChannelTestAccess::arm(*channel, start, start + 4us);
    const std::array<std::byte, 100> payload {};
    REQUIRE_EQ(runtime->queue_message(payload, 0, true, false, 0).status,
        MessageIoStatus::success);
    (void)DatagramChannelTestAccess::poll(*channel, start + 1us);
    const auto packets = take_datagrams(output);
    REQUIRE_EQ(packets.size(), 1U);
    REQUIRE_EQ(decode_packet(packets.front()).packet.kind, PacketKind::data);
}

TEST(compat_paced_continuation_routing_and_runtime_changes_invalidate_hint)
{
    using namespace std::chrono_literals;
    const PacedPollContinuation::Time start {1s};
    auto channel = std::make_shared<DatagramChannel>();
    ...
    const auto invalidated = [&] {
        REQUIRE(!DatagramChannelTestAccess::take(*channel, start + 1us));
    };
    arm();
    REQUIRE(channel->register_connection(42U, runtime));
    invalidated();
    arm();
    runtime->apply_options(SocketOptions {});
    invalidated();
    auto setup = std::make_shared<DatagramInbox>(8U);
    arm();
    REQUIRE(channel->register_setup_inbox(43U, {}, setup));
    invalidated();
    arm();
    channel->unregister_setup_inbox(43U, setup);
    invalidated();
    auto listener = std::make_shared<HandshakeInbox>(8U);
    arm();
    REQUIRE(channel->set_listener_inbox(listener));
    invalidated();
    arm();
    channel->clear_listener_inbox(listener);
    invalidated();
    arm();
    channel->notify_send_work();
    invalidated();
    ...
    invalidated();
    arm();
    channel->unregister_connection(42U);
    invalidated();
}
