...
set -euo pipefail
if [[ $# -ne 5 ]]; then
    echo "usage: $0 OBS_SOURCE BUILD SRT_PREFIX FFMPEG_PREFIX INSTALL_PREFIX" >&2
    exit 2
fi
...
"$repository/tools/python" "$repository/tests/obs/prepare_source.py" "$source_directory"
...
cmake -S "$source_directory" -B "$build" -G Ninja \
    ...
    -DENABLE_FRONTEND=OFF -DENABLE_SCRIPTING=OFF -DENABLE_PLUGINS=ON \
    -DENABLE_NEW_MPEGTS_OUTPUT=ON \
    -DENABLE_WAYLAND=OFF -DENABLE_PULSEAUDIO=OFF
