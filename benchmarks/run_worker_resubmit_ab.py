#!/usr/bin/env python3
"""Fixed 24-case ABBA for the worker-owned scheduler resubmission."""
...
REVISIONS = {
    ...
    "candidate": "264d613543e953c63fd5ebe7d20394a8eb6557a2",
}
STAGE = {
    "connections": 1,
    "bytes_per_connection": 128 * 1024**2,
    "repetitions": 2,
    "warmups": 0,
}
...
def analyze_block(report: dict, manifest: dict, exit_code: int) -> dict:
        ...
        for role in ("sender", "receiver"):
            ...
            for key in RESOURCE_METRICS:
                ...
                row[f"{role}_{key}_per_gib"] = (
                    value * 1024**3 / actual_bytes
                    if common.number(value, integer=True)
                    else None
                )
    for row, case in zip(result["cases"], report.get("runs", [])):
        ...
        for role, side in (("sender", "caller"), ("receiver", "listener")):
            ...
            valid &= (
                type(observed.get("returncode")) is int
...
def analyze_experiment(report: dict) -> dict:
    ...
    contract = (
        ...
        and not report.get("interrupted")
    ...
    if not contract:
        gate["reasons"].append(
            "incomplete/failed 24-case evidence; no candidate nomination"
        )
    ...
    valid_controls = len(controls) == 8 and all(
        common.number(x, positive=True) for x in controls
    )
    result["haivision_controls"] = controls
    result["control_spread"] = max(controls) / min(controls) if valid_controls else None
    if (
        not valid_controls
        or result["control_spread"] > THRESHOLDS["maximum_control_spread"]
    ):
    ...
    for profile in PROFILES:
        ...
        for variant in REVISIONS:
            ...
            if len(rows) != 4 or any(
                not common.number(row.get(k), positive=k in CORE_METRICS)
                for row in rows
                for k in (*CORE_METRICS, *EXTRA_METRICS)
            ):
        ...
        if any(
            ratios[k] > THRESHOLDS["maximum_cpu_ratio"]
            for k in ("sender_cpu_seconds_per_gib", "total_cpu_seconds_per_gib")
        ):
            gate["reasons"].append(f"{profile}: sender/total CPU regression exceeds 5%")
        improved |= (
            ratios["mbps"] >= THRESHOLDS["improved_rate_ratio"]
    ...
    if not improved:
        gate["reasons"].append(
            "no profile improves throughput or total CPU by at least 5%"
        )
    ...
    try:
        for index, variant in enumerate(plan()):
            ...
            active = {
                ...
                "variant": variant,
                "exit_code": None,
                "driver_arguments": argv,
            }
            report["blocks"].append(active)
            sc.write_report(out / "worker-resubmit-report.json", report)
            ...
            try:
                ...
                active["analysis"] = analyze_block(
                    raw, manifests[variant], active["exit_code"]
                )
        ...
        if not report["analysis"]["measurement_contract_pass"]:
            ...
            report["runner_exit_code"] = (
                0 if report["analysis"]["comparison_valid"] else 2
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        parser.error(
            "use the original dedicated srt-network-lab-runtime Linux ARM64 VM"
        )
    ...
    if any(
        int(environment["sysctls"].get(f"net/core/{k}_max") or 0)
        < common.ARGUMENTS["udp_buffer"]
        for k in ("rmem", "wmem")
    ):
    ...
    report = {
        ...
        "harness_files": [
            ...
            for m in (budget, common, diag, sc, build)
        ],
    }
