...
def validate_manifests(manifests: dict, harness: dict, environment: dict,
    ...
    if set(manifests) != set(revisions):
        raise ValueError(f"plain build manifests required for: {', '.join(revisions)}")
    ...
    for variant, manifest in manifests.items():
        ...
        if (manifest.get("transport_trace", {}).get("enabled", False) is not False
                ...
                or manifest.get("continuation_diagnostics", {}).get("enabled", False) is not False):
...
def validate_build_files(paths: dict, manifests: dict) -> dict:
    ...
    if not signatures or any(value != next(iter(signatures.values())) for value in signatures.values()):
...
def analyze_block(report: dict, manifest: dict, exit_code: int,
    ...
    connections = arguments.get("connections", 1)
    for index, entry in enumerate(runs):
        ...
        if (integrity.get("deterministic_payload_verified") is not True
                or integrity.get("verified_connections") != connections
                or result.get("connections") != connections or result.get("message_size_bytes") != 1316
        ...
        if (not number(repeats, integer=True) or not number(original, integer=True, positive=True)
                or original != packets * connections or wire.get("receiver_packets_unique") != packets * connections
